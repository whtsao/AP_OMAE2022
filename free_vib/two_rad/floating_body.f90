...
!===============================================================================
      ...
      IMPLICIT NONE
	  INTEGER I,J,IR,IL,NT,ITR,NGA,NTIM,NFIELD,NPL,NFL,NBT,NNODE,NELM,ICON,NITER,NWG,NDOF,E1LOC,CTYPE
...
!---CLAIM FOR PMTLD
      ...
      REAL*8 DTDT,DVDT,DEDT,DWDT
      REAL*8 DEP2,POR,MU1,MU2,TL,TH,TMW,TWN,COOR2(4,2),FOR2(3),TLDLOC(2),WG1_TLD,WG2_TLD
      REAL*8 MFIX,HFIX,IFIX,A(8)
      REAL*8,ALLOCATABLE::NODE2(:,:),PHI2(:)
      REAL*8,ALLOCATABLE::EQM(:),EQC(:),EQK(:),EQH(:),EQM_ALL(:,:),EQC_ALL(:,:),EQK_ALL(:,:),K_EFF(:,:)
...
!      OPEN(UNIT=12,FILE='DOMAIN.DAT')
      OPEN(UNIT=13,FILE='E.DAT')            ! power of TLD
...
!---TOPOGRAGHY AND WAVE TYPE AND BODY CONFIGURATION
	CALL INPUT_2(NPL,NFL,NBT,DORTYP1,DORTYP2,WAVTYP1,WAVTYP2,AMP1,OMEGA1,PSI1,AMP2,OMEGA2,PSI2,&
                &NWG,WGX,NDOF,BLX,BLY,THOB,MB,IB,Z0_IC,IDOF)
...
!---INPUT PARAMETERS OF THE SLOSHING TANK
    ...
    ALLOCATE(EQM_ALL(NDOFTLD+1,NDOFTLD+1),EQC_ALL(NDOFTLD+1,NDOFTLD+1),EQK_ALL(NDOFTLD+1,NDOFTLD+1),K_EFF(NDOFTLD+1,NDOFTLD+1))
...
!---CALCULATE EQUIVALENT PARAMETERS (BUT ONLY USED IF MECHANICAL MODEL IS APPLIED)
    CALL EQU_PMTLD_SYS(PI,NDOFTLD,GRAV,THO,GAMMA,BETA,DELTTIME,POR,TL,TH,MU1,MB,IB,MFIX,HFIX,IFIX,&
                      &EQM,EQC,EQK,EQH,EQM_ALL,EQC_ALL,EQK_ALL,A,K_EFF)
    
!---GAUSDSIN QUADRATURES AND SHAPE FUNCTION
    CALL GAUSS(WT,RT,NGA)
    CALL SHAP(SHA1,SHA2,SH,NGA,RT)

!---SET RMSE OF PHIT IN CASE CODE STUCK
    E1_1=0.D0
    E1_2=0.D0
...
!---SET EQIVALENT PMTLD HAS SAME DISPL. WITH BODY
    EQZ0=Z0(1)
    EQZ0_PC=EQZ0
...
!---INITIALIZE PMTLD FLOW PATTERN
    IF (TLD_YN==1)THEN
        ...
        CALL PMTLD(NNODE2,NELM2,NGA,ICON2,NELEM2,ME2,NS2,LN2,BUNTYP2,MU1,MU2,POR,&
            ...
            &NODE2,PHI2,FOR2,DTDT,DVDT,DEDT,DWDT)
...
!---OUTPUT INITIAL STATE STRUCTURE
    ...
    WRITE(31,*) 'equivalent mass (kg), frequencies (rad/s), damping ratios'
    DO I=1,NDOFTLD
        WRITE(31,"(I2,3(1X,E15.8))") I,EQM(I),DSQRT(EQK(I)/EQM(I)),0.5D0*EQC(I)/DSQRT(EQK(I)*EQM(I))
    END DO
...
!************** FLOATING BODY DYNAMICS ******************
    IF (TLD_YN==2)THEN
        CALL STR_EQTLD(IDOF,GAMMA,BETA,A,NDOF,NDOFTLD,DELTTIME,MB,IB,TMW,MFIX,HFIX,IFIX,EQM,EQC,EQK,&
                      &EQM_ALL,EQC_ALL,EQK_ALL,EQH,K_EFF,FOR_PC,Z0,Z1,Z2,Z0_PC,Z1_PC,Z2_PC,&
...
WRITE(13,"(5(E15.8,1X))") TIME,DTDT,DVDT,DEDT,-DWDT
...
!--- SOLVE ROTATIONAL MOTION
    ...
    END
!********************************************************************
	SUBROUTINE STR_EQTLD(IDOF,GAMMA,BETA,A,NDOF,NDOFTLD,DELTTIME,MB,IB,TMW,MFIX,HFIX,IFIX,EQM,EQC,EQK,&
                        &EQM_ALL,EQC_ALL,EQK_ALL,EQH,K_EFF,FOR,Z0,Z1,Z2,Z0_PC,Z1_PC,Z2_PC,&
                        &EQZ0,EQZ1,EQZ2,EQZ0_PC,EQZ1_PC,EQZ2_PC,X0_IC,FOR2)
!********************************************************************
! THIS SIMPLE CALCULATION IS ONLY AVAILABLE WHEN NDOF = 1     
	IMPLICIT NONE
    ...
    REAL*8 GAMMA,BETA,DELTTIME,MB,IB,TMW,MFIX,HFIX,IFIX,FOR(3),FOR2(3),A(8)
	REAL*8 Z0(3*NDOF),Z1(3*NDOF),Z2(3*NDOF)
    REAL*8 Z0_PC(3*NDOF),Z1_PC(3*NDOF),Z2_PC(3*NDOF)
...
!--- SOLVE ROTATIONAL MOTION
    IF (IDOF(3)==1)THEN
        ...
        DO I=1,NDOFTLD
            ...
            FOR2(1)=FOR2(1)+EQK(I)*(X0(I+1)-X0(1))+EQC(I)*(X1(I+1)-X1(1)) !+++++++use the kth response++++++
            FOR2(3)=FOR2(3)-(EQK(I)*(X0(I+1)-X0(1))+EQC(I)*(X1(I+1)-X1(1)))*EQH(I)
!            FOR2(1)=FOR2(1)+EQK(I)*(X0_PC(I+1)-X0_PC(1))+EQC(I)*(X1_PC(I+1)-X1_PC(1)) !+++++++use the (k+1)th response++++++
!            FOR2(3)=FOR2(3)-(EQK(I)*(X0_PC(I+1)-X0_PC(1))+EQC(I)*(X1_PC(I+1)-X1_PC(1)))*EQH(I) ! Counterclockwise MOMENT IS + 
        END DO
...
SUBROUTINE PMTLD(NNODE,NELM,NGA,ICON,NELEM,ME,NS,LN,BUNTYP,MU1,MU2,POR,&
                ...
                &NODE,PHI,FOR,DTDT,DVDT,DEDT,DWDT)
IMPLICIT NONE
...
CALL ENERGY_TLD(NNODE,NELEM,NELM,NS,LN,NGA,NODE,PHI,PPHI,PHIT,PPHIT,DP,PR,&
                &JCB,WT,SH,SHA1,SHA2,POR,GRAV,THO,DEP,TMW,Z0,Z1,DTDT,DVDT,DEDT,DWDT)
...
!**********************************************************************
    ...
    REAL*8 JCB(NELM),WT(NGA),SHA1(NGA),SHA2(NGA),SH(2,NGA),Z0(3),Z1(3)
    REAL*8 XFUNC(NGA),YFUNC(NGA),PFUNC(NGA),PPFUNC(NGA)
    REAL*8 PTFUNC(NGA),PPTFUNC(NGA),DPXFUNC(NGA),DPYFUNC(NGA)
    REAL*8 ITA,ITAT,T1,T2,DN1,DN2,S1,S2,TEMP
    REAL*8 POR,GRAV,THO,DTDT,DVDT,DEDT,DWDT,DEP,TMW
...
!-----DV/DT
	...
	DO J=1,NELEM(1)
		...
		DO K=1,2
			DO M=1,NGA
			ITA=(XFUNC(M)-Z0(1))*(-S2)+(YFUNC(M)-Z0(2))*S1 !YFUNC(M)-DEP
            ITAT=DPXFUNC(M)*(-S2)+DPYFUNC(M)*S1
			!PE=PE+ITA*(0.5D0*ITA-DEP)*JCB(J)*SH(K,M)*WT(M)
			DVDT=DVDT+ITA*ITAT*JCB(J)*SH(K,M)*WT(M)
            !DVDT=DVDT+2.D0*ITA*DPYFUNC(M)*JCB(J)*SH(K,M)*WT(M)
			END DO
		END DO
    END DO
	DVDT=DVDT*THO*GRAV+TMW*Z1(2)
...
END DO
    ...
    END
!********************************************************************
	SUBROUTINE EQU_PMTLD_SYS(PI,N,GRAV,THO,GAMMA,BETA,DELTTIME,POR,TL,TH,MU1,MB,IB,MFIX,HFIX,IFIX,&
                            &EQM,EQC,EQK,EQH,EQM_ALL,EQC_ALL,EQK_ALL,A,K_EFF)
!********************************************************************
	IMPLICIT NONE
	INTEGER I,N
