...
!---CLAIM FOR PMTLD
      ...
      REAL*8 KE2,PE2,TE2,WORK2,DTDT2,DVDT2,DEDT2,DWDT2,PE_I2
...
!---OUTPUT INITIAL STATE STRUCTURE
    ...
    DO I=1,NDOFTLD
        WRITE(31,"(I2,3(1X,F15.6))") I,EQM(I),DSQRT(EQK(I)/EQM(I)),0.5D0*EQC(I)/DSQRT(EQK(I)*EQM(I))
    END DO
...
ELSE IF (TLD_YN==2)THEN
    CALL PMTLD_MECHANICAL(NDOFTLD,GRAV,A,TMW,MFIX,IFIX,HFIX,EQM,EQC,EQK,EQH,&
...
!********************************************************************
    ...
    END
!********************************************************************
	SUBROUTINE PMTLD_MECHANICAL(NDOFTLD,GRAV,A,TMW,MFIX,IFIX,HFIX,EQM,EQC,EQK,EQH,&
...
! THE FLOATING BODY IS FULLY COUPLED, THEN ITS RESPONSES ARE TAKEN AS THE GROUND EXCITATIONS TO THE MECHANICAL SYSTEMS
    ...
    REAL*8 GRAV,DELTTIME,TMW,MFIX,IFIX,HFIX
...
!--- SOLVE TLD REACTIONS FX, FY, MZ
    ...
    DO I=1,NDOFTLD
        !--- use the kth response ---
        !FOR2(1)=FOR2(1)+EQK(I)*(X0(I)-Z0(1))+EQC(I)*(X1(I)-Z1(1)) 
        !FOR2(3)=FOR2(3)-EQK(I)*(X0(I)-Z0(1))*EQH(I)-EQC(I)*(X1(I)-Z1(1))*EQH(I)     
        !--- use the (k+1)th response ---
        !FOR2(1)=FOR2(1)+EQK(I)*(X0_PC(I)-Z0(1))+EQC(I)*(X1_PC(I)-Z1(1))
        !FOR2(3)=FOR2(3)-EQK(I)*(X0_PC(I)-Z0(1))*EQH(I)-EQC(I)*(X1_PC(I)-Z1(1))*EQH(I)
        FOR2(1)=FOR2(1)+EQK(I)*X0_PC(I)+EQC(I)*X1_PC(I)
        FOR2(3)=FOR2(3)-EQK(I)*X0_PC(I)*EQH(I)-EQC(I)*X1_PC(I)*EQH(I)
        
        !WRITE(*,*) EQK(I)*X0_PC(I)+EQC(I)*X1_PC(I),-EQM(I)*(Z2(1)+X2_PC(I)-EQH(I)*Z2(3))   
...
SUBROUTINE PMTLD_BEM(NNODE,NELM,NGA,ICON,NELEM,ME,NS,LN,BUNTYP,MU1,MU2,POR,&
                ...
                &NODE,PHI,NODE_TEMP,FOR,KE,PE,TE,WORK,PE0,DTDT,DVDT,DEDT,DWDT)
IMPLICIT NONE
