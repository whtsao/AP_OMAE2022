...
mkdir -p $WORKDIR/Pro_freevib_wall.$SLURM_JOBID
cd $WORKDIR/Pro_freevib_wall.$SLURM_JOBID 
cp $SLURM_SUBMIT_DIR/floating_body.py .
cp $SLURM_SUBMIT_DIR/petsc.options.superlu_dist .
cp $SLURM_SUBMIT_DIR/floating_body.sh .

srun -N2 -n48 -c1 parun --TwoPhaseFlow floating_body.py -l 5 -C "he=0.01" -O petsc.options.superlu_dist
